...
from utils.model import test, get_params, set_params, restore_state_dict
...
class FlowerClient(NumPyClient):
    def __init__(
        ...
        if not os.path.exists(os.path.join(self.log_path, "client_weights")):
            os.mkdir(os.path.join(self.log_path, "client_weights"))

        self.local_model_path = os.path.join(self.log_path, "client_weights", f"client_{self.client_id}.npz")
    ...
    def fit(self, parameters, config):
        ...
        if not os.path.exists(self.local_model_path):    
            ...
            local_params = self.load_local_params()
            params = [lp * 0.80 + p * 0.20 for (lp, p) in zip (local_params, parameters)]
        ...
        np.savez(self.local_model_path, *get_params(self.model))
    ...
    def evaluate(self, parameters: NDArrays, config: Dict[str, Scalar]):
        ...
        if not os.path.exists(self.local_model_path):    
            ...
            local_params = self.load_local_params()
            params = [lp * 0.80 + p * 0.20 for (lp, p) in zip (local_params, parameters)]
            set_params(self.pipeline.model, params)
    ...
    def load_local_params(self):
        ...
        npy_params = restore_state_dict(self.pipeline.model, npy_params)
        return npy_params
    

def get_expp_client_fn(client_ds, train_root: str, val_batches: int, val_batch_size: int, max_steps_for_clients: List[int], context_len: int, pred_len: int, log_path: str, save_dir: str = ""):
